...
let quizAudioHydrationPromise = hydrateQuizAudioUrls();
...
async function ensureQuizAudioUrls() {
  await quizAudioHydrationPromise;

  if (getPlayableQuestions().length === 0) {
    quizAudioHydrationPromise = hydrateQuizAudioUrls();
    await quizAudioHydrationPromise;
  }
}

function getPlayableQuestions() {
  return QUIZ_DATA.filter(question => question.audioUrl);
}
...
io.on('connection', (socket) => {
  ...
  // ── start_game ─────────────────────────────────────────────
  socket.on('start_game', async ({ targetScore = 5 }, cb) => {
    let room;
    try {
      ...
      if (room.isStarting) return cb?.({ error: '음원을 준비하는 중입니다. 잠시만 기다려주세요.' });

      room.isStarting = true;
      await ensureQuizAudioUrls();

      const questions = getRandomQuestions(ROUND_COUNT);
      if (questions.length === 0) {
        return cb?.({
          error: '재생 가능한 음원을 불러오지 못했습니다. Render의 Spotify 환경변수와 Spotify preview_url 제공 여부를 확인해주세요.'
        });
      }
