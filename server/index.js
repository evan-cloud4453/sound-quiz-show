...
const BONUS_PROB     = 0.2   // 보너스 퀴즈 발동 확률
...
function startRound(room) {
  ...
  // ★ 보너스 퀴즈: 5라운드부터(BONUS_MIN_ROUND), 모든 난이도 대상(BONUS_MIN_DIFF=1),
  //   게임당 최대 2회(BONUS_MAX), 20% 확률(BONUS_PROB)
  const diff = Number(question.difficulty) || 1;
  const isBonus = room.currentRound >= BONUS_MIN_ROUND
               && room.bonusUsed < BONUS_MAX
               && diff >= BONUS_MIN_DIFF
               && Math.random() < BONUS_PROB;
